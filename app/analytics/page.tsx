            ...
            {monthlyData.map(d => (
              <div key={d.month} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex gap-1 items-end h-48">
                  <div className="flex-1 bg-indigo-500 rounded-t-md transition-all hover:bg-indigo-600" style={{ height: `${(d.income / maxBar) * 100}%` }} title={`Income: $${d.income.toLocaleString()}`} />
                  <div className="flex-1 bg-rose-400 rounded-t-md transition-all hover:bg-rose-500" style={{ height: `${(d.expenses / maxBar) * 100}%` }} title={`Expenses: $${d.expenses.toLocaleString()}`} />
                </div>
          ...
          <div className="space-y-3">
            {categories.map(cat => (
              <button
                ...
                className={`w-full text-left rounded-lg p-3 transition-all ${selectedCategory === cat.name ? 'bg-gray-50 ring-2 ring-indigo-500' : 'hover:bg-gray-50'}`}
              >
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className={`h-full ${cat.color} rounded-full transition-all`} style={{ width: `${cat.percentage}%` }} />
                </div>
      ...
      <div className="bg-white rounded-xl border p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {insights.map((insight, i) => (
            <div key={i} className={`rounded-lg p-4 ${insight.type === 'positive' ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200'}`}>
              <p className={`text-sm font-semibold ${insight.type === 'positive' ? 'text-green-800' : 'text-amber-800'}`}>{insight.title}</p>
              <p className={`text-xs mt-1 ${insight.type === 'positive' ? 'text-green-600' : 'text-amber-600'}`}>{insight.description}</p>
            </div>
          ))}
        </div>
      </div>
