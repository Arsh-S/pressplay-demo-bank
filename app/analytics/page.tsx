  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Monthly Income</p><p className="text-2xl font-bold text-gray-900 mt-1">${totalIncome.toLocaleString()}</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">YTD Income</p><p className="text-2xl font-bold text-gray-900 mt-1">$14,500</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Projected Annual</p><p className="text-2xl font-bold text-gray-900 mt-1">$58,200</p></div>
      </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="space-y-4">
            {incomeBreakdown.map(src => (
              <div key={src.source}>
                <div className="flex justify-between mb-1"><span className="text-sm font-medium">{src.source}</span><span className="text-sm font-semibold">${src.amount.toLocaleString()}</span></div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full ${src.color} rounded-full`} style={{ width: `${src.percentage}%` }} /></div>
...
function ExpensesTab() {
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Monthly Expenses</p><p className="text-2xl font-bold text-gray-900 mt-1">${totalExpenses.toLocaleString()}</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Daily Average</p><p className="text-2xl font-bold text-gray-900 mt-1">${Math.round(totalExpenses / 30)}</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Largest Expense</p><p className="text-2xl font-bold text-gray-900 mt-1">$650</p><p className="text-xs text-gray-500 mt-1">Shopping</p></div>
      </div>
      <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {categories.map(cat => (
              <button key={cat.name} onClick={() => setSelectedCat(selectedCat === cat.name ? null : cat.name)}
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full ${cat.color} rounded-full`} style={{ width: `${cat.percentage}%` }} /></div>
                {selectedCat === cat.name && (
                  <div className="mt-2 pt-2 border-t text-xs text-gray-500"><p>{cat.percentage}% of total · Avg. ${Math.round(cat.amount / 30)}/day</p></div>
                )}
              </button>
            ))}
          </div>
...
function SavingsTab() {
  return (
    <>
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Monthly Savings</p><p className="text-2xl font-bold text-gray-900 mt-1">$1,650</p><p className="text-xs text-green-600 mt-1">34% savings rate</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Total Saved (YTD)</p><p className="text-2xl font-bold text-gray-900 mt-1">$4,900</p></div>
        <div className="bg-white rounded-xl border p-5"><p className="text-sm text-gray-500">Goals Progress</p><p className="text-2xl font-bold text-gray-900 mt-1">2 of 3</p><p className="text-xs text-gray-500 mt-1">On track</p></div>
      </div>
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <div className="space-y-5">
            {savingsGoals.map(goal => (
              <div key={goal.name}>
                <div className="flex justify-between mb-1"><span className="text-sm font-medium">{goal.name}</span><span className="text-sm text-gray-500">${goal.current.toLocaleString()} / ${goal.target.toLocaleString()}</span></div>
                <div className="h-3 bg-gray-100 rounded-full overflow-hidden"><div className={`h-full ${goal.color} rounded-full transition-all`} style={{ width: `${(goal.current / goal.target) * 100}%` }} /></div>
